...
namespace
{
const float RUN_VX = 380; /**< Horizontal speed while running. */
const float RUN_PINCH_VX = 430; /**< Horizontal speed while running. */
...
Capto::Capto(const ReaderMapping& reader) :
  Boss(reader, "images/creatures/capto/capto.sprite"),
  m_state(),
  m_state_timer(),
  m_safe_timer(),
  m_stomp_count(),
  ...
  m_jumped()
{
  reader.get("hud-icon", m_hud_icon, "images/creatures/capto/hudlife.png");
...
void
Capto::draw(DrawingContext& context)
{
...
void
Capto::active_update(float dt_sec)
{
...
HitResponse
Capto::collision_badguy(BadGuy& badguy, const CollisionHit& hit)
{
  badguy.kill_fall();
  return FORCE_MOVE;
}
...
void
Capto::kill_squished(GameObject& object)
{
...
void
Capto::collision_solid(const CollisionHit& hit)
{
...
Capto::SnowExplosionParticle::SnowExplosionParticle(const Vector& pos, const Vector& velocity)
...
Capto::get_allowed_directions() const
{
  return {};
}
