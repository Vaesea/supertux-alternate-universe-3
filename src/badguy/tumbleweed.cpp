...
static const float JUMPSPEED = -380;
static const float BSNOWBALL_WALKSPEED = 150;

Tumbleweed::Tumbleweed(const ReaderMapping& reader) :
  BadGuy(reader, "images/creatures/tumbleweed/tumbleweed.sprite"),
  m_x_speed(),
  m_turn_around_timer(),
...
Tumbleweed::Tumbleweed(const Vector& pos, Direction d, float x_vel) :
  BadGuy(pos, d, "images/creatures/tumbleweed/tumbleweed.sprite"),
  m_x_speed(),
  m_turn_around_timer(),
  m_turn_around_counter()
{
...
void
Tumbleweed::active_update(float dt_sec)
{
...
std::string
Tumbleweed::get_default_sprite_name() const
{
  switch (m_type)
  {
    case FATBAT:
...
bool
Tumbleweed::collision_squished(GameObject& object)
{
  if (m_frozen)
...
void
Tumbleweed::collision_solid(const CollisionHit& hit)
{
...
HitResponse
Tumbleweed::collision_badguy(BadGuy& , const CollisionHit& hit)
{
